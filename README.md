...
- [`docs/agent-core-vps-deploy-plan.md`](docs/agent-core-vps-deploy-plan.md) — deploy VPS Ubuntu qua Docker, app/API trên 2 domain riêng (vd. `app-harness.../api-harness...`) — biến `VITE_REST_URL`/`VITE_WS_URL` (build-time) + service `api-gateway` gộp REST+WS 1 port, đã verify thật (build bundle + WS handshake qua gateway)
