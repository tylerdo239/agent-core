...
export class AgentRunner extends AgentRunnerService {
  async runTurn(driverName: string, session: Session, userMessage: string): Promise<LoopTurnResult> {
    ...
    // Memory integration: cùng lý do ghi ở entrypoint chung này (coding rule
    // B4) thay vì trong từng loop driver. `ctx.memory` KHÔNG có trong
    // `inject` bên dưới -- seam optional, chỉ mount khi MEMORY_CORE_URL được
    // cấu hình (src/serve.ts).
    //
    ...
    this.ctx.get('memory')?.remember(session.id, userMessage, { userId: session.ownerId }).catch(() => {})
