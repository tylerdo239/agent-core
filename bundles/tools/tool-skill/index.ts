...
export const apply = (ctx: Context) => {
  ctx.tools.add({
    ...
    async handler(args, invocation) {
      ...
      const definition = ctx.skills.get(name)
      if (!definition) {
        const available = ctx.skills.list({ topLevelOnly: true }).map((item) => item.name)
  ...
  ctx.tools.add({
    ...
    async handler(args, invocation) {
      ...
      const resource = await ctx.skills.readResource(name, resourcePath)
      const event = {
