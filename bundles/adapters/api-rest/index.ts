...
//
// Production hardening: MỌI request (trừ /health, /ready — orchestrator cần
// gọi được mà không có key) phải có `Authorization: Bearer <key>` hợp lệ qua
...
export const inject = ['sessions', 'agent', 'storage', 'auth', 'skills', 'workspace']
...
async function handle(ctx: Context, req: IncomingMessage, res: ServerResponse, maxBodyBytes: number) {
  ...
  if (req.method === 'GET' && pathname === '/ready') {
    ...
    const ready = ['sessions', 'agent', 'storage'].every((name) => ctx.reflect.get(name, false) !== undefined)
    return sendJson(res, ready ? 200 : 503, { ready })
  }

  if (!PUBLIC_PATHS.has(pathname) && !ctx.auth.verify(extractBearerToken(req))) {
    return sendJson(res, 401, { error: 'unauthorized' })
  }

  if (req.method === 'POST' && pathname === '/ws-ticket') {
    const ticket = ctx.auth.issueTicket?.()
    return ticket ? sendJson(res, 201, ticket) : sendJson(res, 501, { error: 'ticket authentication is not supported' })
  }
