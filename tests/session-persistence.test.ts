...
describe('session persistence', () => {
  it('restores metadata and conversational history after restart', async () => {
    ...
    const session = first.sessions.create({ id: 'persistent', driver: 'default', systemPrompt: 'system rule', ownerId: 'user-123', projectId: 'project-123' })
    ...
    try {
      ...
      expect(restored?.projectId).toBe('project-123')
