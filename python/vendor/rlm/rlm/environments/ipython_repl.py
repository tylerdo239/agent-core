...
class _SubcallBroker:
    ...
    ``subcall_fn`` finishes after that cell has timed out):
        ...
        {"type": "tool", "name": str, "args": dict, "cell_id": str}
    ...
    Responses:
        ...
        tool:             {"result": JSON-compatible value}          | {"error": str}
        answer:           {"ok": True}
    """
    ...
    def __init__(
        ...
        tool_call_fn: Callable[[str, dict[str, Any]], Any] | None = None,
        skill_read_fn: Callable[[str, str], Any] | None = None,
        ...
        self.skill_read_fn = skill_read_fn
    ...
    def _dispatch(self, data: dict[str, Any]) -> dict[str, Any]:
        ...
        if self._shutting_down and req_type in ("subcall", "subcall_batched", "tool", "skill_resource"):
        ...
        if req_type == "tool":
            if self.tool_call_fn is None:
                return {"error": "No tool_call_fn configured; host tools unavailable"}
            name = data.get("name")
            args = data.get("args")
            ...
            try:
                return {"result": self.tool_call_fn(name, args)}
            except Exception as e:
                return {"error": f"{type(e).__name__}: {e}"}

        if req_type == "skill_resource":
            if self.skill_read_fn is None:
                return {"error": "No skill_read_fn configured; skill resources unavailable"}
            skill = data.get("skill")
            resource_path = data.get("path")
            if not isinstance(skill, str) or not isinstance(resource_path, str):
                return {"error": "Skill and resource path must be strings"}
            try:
                return {"resource": self.skill_read_fn(skill, resource_path)}
            except Exception as e:
                return {"error": f"{type(e).__name__}: {e}"}
...
def _build_kernel_bootstrap(
    ...
    return textwrap.dedent(
        ...
        def _rlm_host_tool(name, args=None, **kwargs):
            if args is None:
                payload = {{}}
            elif isinstance(args, dict):
                payload = dict(args)
            else:
                raise TypeError("host tool accepts one dict argument and/or keyword arguments")
            payload.update(kwargs)
            resp = _rlm_request(_RLM_SUBCALL_ADDRESS, {{
                "type": "tool", "name": str(name), "args": payload,
                "cell_id": _RLM_CURRENT_CELL,
            }})
            if not isinstance(resp, dict):
                raise RuntimeError("Host tool returned a malformed response")
            if resp.get("error"):
                raise RuntimeError(resp["error"])
            return resp.get("result")

        def _rlm_skill_resource(skill, path):
            resp = _rlm_request(_RLM_SUBCALL_ADDRESS, {{
                "type": "skill_resource", "skill": str(skill), "path": str(path),
                "cell_id": _RLM_CURRENT_CELL,
            }})
            if not isinstance(resp, dict):
                raise RuntimeError("Skill resource returned a malformed response")
            if resp.get("error"):
                raise RuntimeError(resp["error"])
        ...
        def _RLM_CAPTURE_ANSWER():
            current = get_ipython().user_ns.get("answer")
            if _RLM_SUBCALL_ADDRESS is not None and isinstance(current, dict) and current.get("ready"):
                try:
                    resp = _rlm_request(_RLM_SUBCALL_ADDRESS, {{
                        "type": "answer",
                    ...
                    if isinstance(resp, dict) and resp.get("ok"):
                        # Consume the submitted answer so a completed turn's
                        # ready=True can never be re-captured as a stale final
                        # answer by a later cell or user turn.
                        get_ipython().user_ns["answer"] = _RLMAnswerDict()
                except Exception:
                    pass
...
class IPythonREPL(NonIsolatedEnv):
    ...
    Args:
        ...
        compaction: If True, expose the running compacted root trajectory as
            ``history`` while preserving full per-turn histories as
            ``history_N``.
    """
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.subcall_fn = subcall_fn
        self.tool_call_fn = tool_call_fn
        self.host_tools = list(host_tools or [])
        self._host_tool_names: set[str] = set()
        self.skill_read_fn = skill_read_fn
        self.active_skill = dict(active_skill or {})
        ...
        # Host-side canonical trajectory used by RLM context compaction.
        # The kernel receives a copy/reference as ``history``; keeping the
        # canonical value here prevents model code from corrupting it.
        ...
        try:
            ...
            if self.compaction:
                self._inject_compaction_history()
    ...
    def _setup_in_process(self) -> None:
        ...
        ns["host_tool"] = self._host_tool
        ns["skill_resource"] = self._skill_resource

        # Inject custom tools
        for name, entry in self.custom_tools.items():
            ns[name] = extract_tool_value(entry)
        self._install_host_tools_in_process()
    ...
    def _setup_subprocess(self) -> None:
        ...
        self._broker = _SubcallBroker(
            ...
            skill_read_fn=self.skill_read_fn,
        ...
        self._install_host_tools_subprocess()
        self._install_active_skill_subprocess()
    ...
    @staticmethod
    def _valid_host_tool_names(tools: list[dict[str, Any]]) -> list[str]:
        names: list[str] = []
        for tool in tools:
            ...
            if (
                ...
                and name not in RESERVED_TOOL_NAMES
                and name not in names
            ):
                names.append(name)
        return names

    def _host_tool(self, name: str, args: dict[str, Any] | None = None, **kwargs: Any) -> Any:
        if self.tool_call_fn is None:
            raise RuntimeError("No tool_call_fn configured; host tools unavailable")
        payload = dict(args or {})
        payload.update(kwargs)
        return self.tool_call_fn(str(name), payload)

    def _skill_resource(self, resource_path: str) -> Any:
        if self.skill_read_fn is None:
            raise RuntimeError("No skill_read_fn configured; skill resources unavailable")
        skill_name = self.active_skill.get("name")
        allowed = {
            item.get("path")
            for item in self.active_skill.get("resources", [])
            if isinstance(item, dict)
        }
        if not isinstance(skill_name, str) or not skill_name:
            raise RuntimeError("No active skill for this turn")
        if resource_path not in allowed:
            raise RuntimeError(
                f"Resource {resource_path!r} was not exposed by skill {skill_name!r}"
            )
        return self.skill_read_fn(skill_name, str(resource_path))

    def _install_host_tools_in_process(self) -> None:
        if self._shell is None:
            return
        ns = self._shell.user_ns
        for name in self._host_tool_names:
            ns.pop(name, None)
        names = self._valid_host_tool_names(self.host_tools)
        for name in names:
            def _proxy(args=None, _name=name, **kwargs):
                return self._host_tool(_name, args, **kwargs)
            ns[name] = _proxy
        self._host_tool_names = set(names)

    def _install_host_tools_subprocess(self) -> None:
        if self._kc is None:
            return
        names = self._valid_host_tool_names(self.host_tools)
        lines = [f"globals().pop({name!r}, None)" for name in self._host_tool_names]
        for name in names:
            lines.extend([
                f"def {name}(args=None, **kwargs):",
                f"    return _rlm_host_tool({name!r}, args, **kwargs)",
            ])
        if lines:
            result = self._execute_in_kernel("\n".join(lines), drain_broker=False)
            if result.stderr:
                raise RuntimeError(f"Failed to inject host tools: {result.stderr}")
        self._host_tool_names = set(names)

    def _install_active_skill_subprocess(self) -> None:
        if self._kc is None:
            return
        skill_name = self.active_skill.get("name")
        resources = [
        ...
        code = textwrap.dedent(
            f"""
            _RLM_ACTIVE_SKILL = {skill_name!r}
            _RLM_ACTIVE_SKILL_RESOURCES = {resources!r}
            def skill_resource(path):
                if _RLM_ACTIVE_SKILL is None:
                    raise RuntimeError("No active skill for this turn")
                if path not in _RLM_ACTIVE_SKILL_RESOURCES:
                    raise RuntimeError(f"Resource {{path!r}} is not exposed by the active skill")
                return _rlm_skill_resource(_RLM_ACTIVE_SKILL, path)
            """
        ).strip()
        result = self._execute_in_kernel(code, drain_broker=False)
        if result.stderr:
            raise RuntimeError(f"Failed to inject active skill: {result.stderr}")

    def set_host_tools(self, tools: list[dict[str, Any]]) -> None:
        """Replace Python proxies while handlers remain in the host registry."""
        self.host_tools = list(tools or [])
        if self.kernel_mode == "in_process":
            self._install_host_tools_in_process()
        else:
            self._install_host_tools_subprocess()

    def set_active_skill(self, skill: dict[str, Any] | None) -> None:
        self.active_skill = dict(skill or {})
    ...
    def add_history(
        ...
        """Store a message history as ``history_N``.

        Outside compaction mode, ``history`` aliases ``history_0``. In
        compaction mode, ``history`` is reserved for the running compacted
        trajectory, matching LocalREPL and DockerREPL.
        """
    ...
    def _inject_compaction_history(self) -> None:
        """Refresh the kernel's ``history`` from the host-side journal."""
        payload = copy.deepcopy(self._compaction_history)
        ...
        history_path = os.path.join(self.temp_dir, "_compaction_history.json")
        with open(history_path, "w") as f:
            json.dump(payload, f)
        code = (
            "import json as _rlm_json\n"
            f"with open(r'{history_path}', 'r') as _rlm_f:\n"
            "    history = _rlm_json.load(_rlm_f)"
        )
        result = self._execute_in_kernel(code, drain_broker=False)
        if result.stderr:
            raise RuntimeError(f"Failed to inject compaction history: {result.stderr}")
        self._subprocess_shadow["history"] = payload

    def append_compaction_entry(self, entry: list[dict[str, Any]] | dict[str, Any]) -> None:
        """Append a trajectory segment or summary to compaction ``history``.

        The host owns the canonical journal and refreshes the kernel alias so
        model code cannot permanently overwrite accumulated compaction state.
        """
    ...
    def _execute_in_kernel_locked(
        ...
        # A model may rebind ``answer`` to a plain dict instead of mutating the
        # scaffolded _RLMAnswerDict. Capture that valid completion form before
        # draining the broker, so ready=True ends the current iteration.
        if drain_broker and not timed_out:
            try:
                self._kc.execute_interactive(
                    "_RLM_CAPTURE_ANSWER()",
                    timeout=self.startup_timeout,
                ...
                stderr_parts.append("\nTimeoutError: answer capture exceeded startup timeout")
    ...
    def _restore_scaffold_in_process(self) -> None:
        ...
        if self.compaction:
            ns["history"] = copy.deepcopy(self._compaction_history)
        elif "history_0" in ns:
