            ...
            try:
                for i in range(self.max_iterations):
                    ...
                    if final_answer is not None:
                        ...
                        # Persist the terminal iteration too. Previously the early return
                        # stored only the history accumulated *before* this iteration, so a
                        # later turn could inspect every intermediate step but not the final
                        # report that it needed to review or continue from.
                        final_messages = format_iteration(iteration)
                        message_history.extend(final_messages)
                        if self.compaction and hasattr(
                            environment, "append_compaction_entry"
                        ):
                            environment.append_compaction_entry(final_messages)

                        # Store the now-complete message history in the persistent environment.
