...
def find_code_blocks(text: str) -> list[str]:
    ...
    """
    # OpenAI-compatible chat templates do not consistently preserve the
    # requested fence label: the same executable action may arrive as
    # ```repl or ```python. Both represent notebook actions in an RLM model
    # response and still pass through the runtime policy before execution.
    pattern = r"```(?:repl|python)\s*\n(.*?)\n```"
    results = []

    for match in re.finditer(pattern, text, re.DOTALL | re.IGNORECASE):
        code_content = match.group(1).strip()
        # Some models repeat the fence label ("repl"/"python") as the first
        # line inside the block, or wrap the body in an XML-style tag such as
        # "<repl>". Executing those lines raises NameError/SyntaxError, so
        # normalize them away before returning the block.
        lines = code_content.split("\n")
        while lines and lines[0].strip().casefold().strip("<>") in {"repl", "python"}:
            lines = lines[1:]
            code_content = "\n".join(lines).strip()
        lines = code_content.split("\n")
