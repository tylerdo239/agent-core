...
RLM_SYSTEM_PROMPT = textwrap.dedent(
    """You are a Recursive Language Model (RLM). The current request may be stated directly in the user message, while optional data and runtime state are stored in a persistent Python REPL. Use the cheapest sufficient path: answer from the visible request when no runtime evidence is needed, and inspect or compute in the REPL only when it materially improves correctness.
You can iteratively interact with a Python REPL, which has access to LLM calls as functions. You will be queried turn-by-turn until you have an answer to the query.

To use the REPL, you need to write code in ```repl``` blocks; the REPL persists across turns. Available in the REPL:
- `context`: a legacy alias for the immutable `context_0`. The alias and numeric suffixes describe
  storage only; they do not assign task roles or relevance. In a persistent session, later inputs
  are stored separately as `context_1`, `context_2`, and so on.
- `history`: prior completed RLM trajectories when available. It contains earlier model actions,
  REPL outputs, findings, and answers.
...
REPL outputs over ~20K characters are truncated, so inspect `context`, later `context_N` values,
and `history` selectively. Print keys, metadata, lengths, or small slices instead of entire
datasets or trajectories; pass focused slices through `llm_query` when semantic interpretation is
needed. The REPL is NOT a Jupyter cell — only `print(...)` output (stdout) is shown back to you
between turns; a bare expression on the last line is silently discarded. Always wrap inspections
in `print(...)`.

When the current request is stated directly in the user message, do not inspect `context_N` merely
to rediscover it. Read the newest relevant context only when you need runtime state absent from the
message, such as datasets, session memory, tool metadata, selected-skill content, or pending human
control. Treat other contexts as versioned values with no implied task role. Use prior raw history
only to recover an exact missing detail that semantic memory does not contain.

Every iteration must contain exactly one ```repl``` block; prose alone makes no progress. For a
simple request that needs no runtime evidence, submit a concise answer in the first block by setting
`answer["content"]` and `answer["ready"]`. For investigative work, state one concise next action,
execute it, inspect only focused output, and continue. Never repeat an identical inspection after it
produces no useful output; change approach. Do not expose private chain-of-thought or protocol
debugging as the user-facing answer.
"""
)
...
ORCHESTRATOR_ADDENDUM = "\n\n".join(
    [
        ...
        (
            "Directly after you probe the task-relevant context and understand the current task, "
            "pause and plan: "
...
def build_user_prompt(
    ...
    if iteration == 0:
        if root_prompt:
            safeguard = (
                "The current request is stated directly above. Do not inspect a context merely "
                "to rediscover it. Use the REPL only for runtime state or evidence the request "
                "actually needs. If none is needed, submit the answer now in one `repl` block.\n\n"
            )
        elif context_count > 1:
            safeguard = (
                f"This persistent session has a new input in `context_{context_count - 1}`. "
                "No direct request was supplied, so inspect only its `request`, `type`, and any "
                "runtime fields needed for the task. Other contexts are versioned values with no "
                "implied task role; use only relevant ones."
            )
            if history_count > 0:
                safeguard += (
                    " Continue from any model-visible semantic session summary. Inspect a relevant "
                    "raw `history` entry only when exact missing detail must be recovered."
                )
            safeguard += "\n\n"
        else:
            safeguard = (
                "No direct request was supplied. Inspect the only available context selectively "
                "to identify the request and any required runtime state.\n\n"
            )
        prompt = safeguard + body
    else:
        prompt = (
            body
            + " Emit exactly one `repl` block: either perform one new useful action or submit "
            "the final answer. Do not narrate, repeat an unchanged inspection, or wait without "
            "executing code."
        )
